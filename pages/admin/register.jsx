...
const register = () => {
    ...
    const { admin_register } = useContext(AuthContext)
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await admin_register(formData)
            ...
            setTimeout(() => {
                setLoading(false);
                if(response?.data?.role === "1") router.push(`/admin/dashboard?profil=${formData.role}`);
